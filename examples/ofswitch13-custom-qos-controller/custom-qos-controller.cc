...
CustomQosController::CustomQosController ()
{
...
TypeId
CustomQosController::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CustomQosController")
    .SetParent<OFSwitch13Controller> ()
    .SetGroupName ("OFSwitch13")
    .AddConstructor<CustomQosController> ()
    ...
    .AddAttribute ("MeterRate",
                   ...
                   MakeDataRateAccessor (&CustomQosController::m_meterRate),
                   MakeDataRateChecker ())
    ...
    .AddAttribute ("ServerIpAddr",
                   ...
                   MakeAddressAccessor (&CustomQosController::m_serverIpAddress),
                   MakeAddressChecker ())
    .AddAttribute ("ServerTcpPort",
                   "Server TCP port.",
                   UintegerValue (9),
                   MakeUintegerAccessor (&CustomQosController::m_serverTcpPort),
                   MakeUintegerChecker<uint64_t> ())
    .AddAttribute ("ServerMacAddr",
                   "Server MAC address.",
                   AddressValue (Address (Mac48Address ("00:00:00:00:00:01"))),
                   MakeAddressAccessor (&CustomQosController::m_serverMacAddress),
                   MakeAddressChecker ())
  ;
  return tid;
}
...
void
CustomQosController::HandshakeSuccessful (Ptr<const RemoteSwitch> swtch)
{
  NS_LOG_FUNCTION (this << swtch);
...
void
CustomQosController::ConfigureBorderSwitch (Ptr<const RemoteSwitch> swtch)
{
  NS_LOG_FUNCTION (this << swtch);
...
ofl_err
CustomQosController::HandleArpPacketIn (
  struct ofl_msg_packet_in *msg, Ptr<const RemoteSwitch> swtch,
  uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
ofl_err
CustomQosController::HandleConnectionRequest (
  struct ofl_msg_packet_in *msg, Ptr<const RemoteSwitch> swtch,
  uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
Ipv4Address
CustomQosController::ExtractIpv4Address (uint32_t oxm_of, struct ofl_match* match)
{
  switch (oxm_of)
    {
    case OXM_OF_ARP_SPA:
...
Ptr<Packet>
CustomQosController::CreateArpRequest (Mac48Address srcMac, Ipv4Address srcIp,
                                 Ipv4Address dstIp)
{
...
Ptr<Packet>
CustomQosController::CreateArpReply (Mac48Address srcMac, Ipv4Address srcIp,
...
void
CustomQosController::SaveArpEntry (Ipv4Address ipAddr, Mac48Address macAddr)
{
